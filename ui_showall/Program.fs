...
do
    ...
    if roleForm.ShowDialog() = DialogResult.OK then
        ...
        if isAdmin then
            ...
            btnLogin.Click.Add(fun _ ->
                ...
                | Some u ->
                    ...
                    loginForm.Close()
                | None ->
                    lblStatus.Text <- "Invalid admin credentials!"
            )
        ...
        let roleText =
            if isAdmin then
                sprintf "Admin (%s)" currentUser.Value.Username
            else
                "User"
        ...
        let btnAdd =
            if isAdmin then makeButton("Add Word", 100, 20, true, adminColor) else null
        let btnUpdate =
            if isAdmin then makeButton("Update Word", 100, 200, true, adminColor) else null
        let btnDelete =
            if isAdmin then makeButton("Delete Word", 100, 380, true, adminColor) else null
        ...
        btnSearchExact.Click.Add(fun _ ->
            if String.IsNullOrWhiteSpace(txtKey.Text) then
                MessageBox.Show("Please enter a word!") |> ignore
            else
                lst.Items.Clear()
                let dict = readDictionary()
                match searchExact txtKey.Text dict with
        ...
        let controls =
            [ lblCurrentUser :> Control
              ...
              lblValue :> Control; txtValue :> Control ]
            @ (if isAdmin then [btnAdd :> Control; btnUpdate :> Control; btnDelete :> Control] else [])
            @ [ btnSearchExact :> Control
                btnSearchPartial :> Control
                btnShowAll :> Control
                lst :> Control ]
